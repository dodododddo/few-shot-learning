...
def main(config):
    ...
    if not args.sauc:
        ...
        n_way = 2
    n_shot, n_query = args.shot, config['n_query'] if config.get('n_query') else 15
    n_batch = config['n_batch'] if config.get('n_batch') else 200
    ...
    if config.get('load'):
        ...
        if config.get('load_encoder'):
           model.encoder.load_state_dict(torch.load(config['load_encoder'])['model_sd'], strict=False)
